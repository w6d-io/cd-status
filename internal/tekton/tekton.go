...
func (t *Tekton) GetWatch(ctx context.Context, kind string, name string) (w watch.Interface) {
    log := logx.WithName(ctx, "GetWatch").WithValues("object", t.PipelineRun.NamespacedName.String(), "kind", kind)
    namespace := t.PipelineRun.NamespacedName.Namespace
    //tknParam := cli.TektonParams{}
    var err error
    opts := metav1.ListOptions{
        FieldSelector:  fmt.Sprintf("metadata.name=%s", name),
        TimeoutSeconds: pointer.Int64(Timeout.Milliseconds() / 1000),
    }
...
func Condition(c v1.Conditions) (status string, reason string) {
    if len(c) == 0 {
        return "---", ""
    }
...
func IsTerminated(c v1.Conditions) bool {
    if len(c) == 0 {
        return false
    }
    switch c[0].Status {
