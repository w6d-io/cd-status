...
func (t *Tekton) SupTasks(ctx context.Context, pr *tkn.PipelineRun) {
    ...
    for _, task := range t.PipelineRun.GetTasks(ctx, pr) {
        ...
        if err := t.TaskRunSupervise(ctx, nn); err != nil {
...
func (p *PipelineRunPayload) SetCondition(ctx context.Context, c v1.Conditions) {
    logx.WithName(ctx, "SetCondition").V(1).Info("call")
...
func (p *PipelineRunPayload) GetTasks(ctx context.Context, pr *tkn.PipelineRun) (ts Tasks) {
    ...
    if len(pr.Status.TaskRuns) > 0 {
        for taskRunName, taskrunStatus := range pr.Status.TaskRuns {
            ts = append(ts, p.GetTask(ctx, taskRunName, taskrunStatus))
        }
        sort.Sort(ts)
    }
...
func (PipelineRunPayload) GetTask(ctx context.Context, taskRunName string, taskrunStatus *tkn.PipelineRunTaskRunStatus) Task {
