...
// TaskRunSupervise watches all pod event created by pipelinerun
func (t *Tekton) TaskRunSupervise(ctx context.Context, nn types.NamespacedName) error {
    log := logx.WithName(ctx, "TaskRunSupervise").WithValues("object",
        t.PipelineRun.NamespacedName.String(),
        "name", nn.Name)
    w := t.GetWatch(ctx, "taskrun", nn.Namespace)
    if w == nil {
    ...
    log.WithValues("timeout", Timeout).V(1).Info("timeout set")
    ...
    for {
        ...
        case e := <-w.ResultChan():
            if e.Object == nil {
                ...
                if err := hook.Send(context.TODO(), t.PipelineRun, "timeout"); err != nil {
            ...
            t.UpdatePayloadTask(trp.GetTask(ctx, tr))

            if err := hook.Send(context.TODO(), t.PipelineRun, "update"); err != nil {
                log.Error(err, "hook failed")
                return err
            }
...
func (TaskRunPayload) GetTask(ctx context.Context, taskRun *tkn.TaskRun) Task {
    log := logx.WithName(ctx, "GetTask")
    status := taskRun.Status
    steps := GetSteps(ctx, status)
