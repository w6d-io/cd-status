...
func Play(c *gin.Context) {
    ...
    go func(kind string, nn types.NamespacedName, projectID, pipelineID int64) {
        err := scan(ctx, nn, projectID, pipelineID,
            payload.Commit.SHA, payload.Commit.Message, payload.Commit.Ref,
            payload.RepoURL)
        if err != nil {
            log.WithValues("kind", kind).Error(err, "Scan resource")
...
func AddWatcher(name string, f func(context.Context, types.NamespacedName, int64, int64, string, string, string, string) error) {
    scans[name] = f
}
